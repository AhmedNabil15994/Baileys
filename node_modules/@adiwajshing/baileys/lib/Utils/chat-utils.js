...
const chatModificationToAppPatch = (mod, jid) => {
    ...
    if ('mute' in mod) {
    ...
    else if ('starMessage' in mod) {
        patch = {
            syncAction: {
                starAction: {
                    starred: mod.starMessage.starred,
                }
            },
            index: ['star', jid , mod.starMessage.messageId, '0' , '0'],
            type: 'regular_high',
            apiVersion: 2,
            operation: OP.SET
        };
        ...
        patch = {
            ...
            index: ['label_jid',mod.labelAssociation.label_id,mod.labelAssociation.jid],
            type: 'regular',
            apiVersion: 3,
            operation: OP.SET,
        };
    }
    else if ('labelMessage' in mod) {
        patch = {
            syncAction: {
                labelAssociationAction: {
                    labeled: mod.labelMessage.labeled,
                }
            },
            index: ['label_message', mod.labelMessage.label_id ,mod.labelMessage.jid , mod.labelMessage.messageId, mod.labelMessage.fromMe,'0'],
            type: 'regular',
            apiVersion: 3,
            operation: OP.SET,
        };
    }
    else if ('labelEdit' in mod) {
        patch = {
            syncAction: {
                labelEditAction: {
                    name: mod.labelEdit.name, 
                    color: mod.labelEdit.color, 
                    deleted: mod.labelEdit.deleted,
                }
            },
            index: ['label_edit', mod.labelEdit.label_id],
            type: 'regular',
            apiVersion: 3,
            operation: OP.SET,
        };
        ...
        patch = {
            syncAction: {
                quickReplyAction: {
                    ...
                    keywords: [],
                    count: 0,
                    deleted: mod.quickReply.deleted
                }
            },
            index: ['quick_reply', mod.quickReply.id],
            type: 'regular',
            apiVersion: 3,
            operation: OP.SET,
        };
...
const processSyncAction = (syncAction, ev, me, initialSyncOpts, logger) => {
    ...
    // console.log(syncAction.syncAction.value)
    if (action === null || action === void 0 ? void 0 : action.muteAction) {
        ...
        if(action.labelEditAction.deleted === true){
            itemObj.deleted = true
            ev.emit('labels.delete',[itemObj])
        }else{
            itemObj.title = action.labelEditAction.name
            itemObj.color = action.labelEditAction.color
            ev.emit('labels.set',[itemObj])
        }
    }
    else if (action === null || action === void 0 ? void 0 : action.quickReplyAction) {
        let itemObj = {
            id: id, 
        }
        if(action.quickReplyAction.deleted === true){
            itemObj.deleted = true
            ev.emit('quick_reply.delete',[itemObj])
        }else{
            itemObj.keywords = action.quickReplyAction.keywords
            itemObj.shortcut = action.quickReplyAction.shortcut
            itemObj.message = action.quickReplyAction.message
            itemObj.count = action.quickReplyAction.count
            ev.emit('quick_reply.set',[itemObj])
        }
    }
    else {
