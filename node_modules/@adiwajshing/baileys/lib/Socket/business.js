...
const makeBusinessSocket = (config) => {
    ...
    const productUpdate = async (productId, update) => {
        if(update.hasOwnProperty('images')){
    ...
    const labelUpdate = async (labelId, label) => {
        // const editNode = (0, business_1.toLabelNode)(labelId, update);
        ...
        //     ]
        // });
        // const productCatalogEditNode = (0, generic_utils_1.getBinaryNodeChild)(result, 'label_edit');
        let myContent = [];
        if (typeof labelId !== 'undefined') {
            myContent.push({
                tag: 'id',
                attrs: {},
                content: Buffer.from(labelId)
            });
        }
        if (typeof label.name !== 'undefined') {
            myContent.push({
                tag: 'name',
                attrs: {},
                content: Buffer.from(label.name)
            });
        }
        if (typeof label.color !== 'undefined') {
            myContent.push({
                tag: 'color',
                attrs: {},
                content: Buffer.from(label.color)
            });
        }
        if (typeof label.deleted !== 'undefined') {
            myContent.push({
                tag: 'deleted',
                attrs: {},
                content: Buffer.from(label.deleted.toString())
            });
        }
        let myNode = {
                tag: 'iq',
        ...
        console.log(myNode.content[0].content[0].content[0].content)
        const result = await query(myNode);
        console.log(result)
        return result
        // const productNode = (0, generic_utils_1.getBinaryNodeChild)(productCatalogEditNode, 'product');
        // return (0, business_1.parseProductNode)(productNode);
